...
<div class="card-glass mb-6 w-full max-w-md hover:scale-102 transition-transform" style="animation: var(--animate-float);">
  <div class="flex items-start gap-4">
    <!-- Profile Image -->
    <div class="glass-sm h-24 w-24 rounded-full overflow-hidden flex items-center justify-center">
      {#if user.email}
        <img 
    ...
    <div class="flex-1">
      ...
      {#if showSimilarity && similarityScore !== null}
        <div class="mt-3 glass-sm p-2 rounded-lg">
  ...
  <div class="mt-4 flex justify-end gap-2">
    <a 
      href={`/users/i?i=${user.id}`}
      class="glass-button px-4 py-2 text-sm text-primary font-medium"
    >
      View Profile
    </a>
